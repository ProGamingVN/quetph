...
> Cập nhật lần cuối: rà soát + sửa **Chế độ Quét trực tiếp** (`live.html` + `live.js` — camera live + khung vuông hướng dẫn + tự động chụp, đã có sẵn từ trước nhưng chưa được ghi vào file này): sửa lỗi tự chụp đè sau khi đã chụp xong, thêm toggle dùng thang đo Calibration cho đồng bộ với trang Quét mẫu. Chuẩn bị README.md + .gitignore để đăng GitHub Pages.
