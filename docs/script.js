(() => {
  ...
  /* Chiều rộng hiển thị tối đa của canvas — PHẢI lấy đúng bề rộng thật sự
     mà .stage nhường cho canvas, KHÔNG hard-code 1 số cố định như trước
     (640). Lý do: mọi phần tử đè lên canvas (kính ngắm, tay kéo, nhãn bán
     kính) đều được định vị bằng toạ độ PIXEL NỘI BỘ của canvas (canvas.width),
     coi 1 canvas-px = 1 CSS-px hiển thị. Điều này chỉ đúng khi canvas KHÔNG
     bị CSS `max-width:100%` co nhỏ thêm lần nữa. Hằng số 640 lớn hơn hẳn bề
     rộng khung hình thật trên điện thoại (thường ~300-350px sau padding),
     nên canvas bị max-width co nhỏ lại trong khi kính ngắm vẫn định vị theo
     toạ độ 640px gốc → kính ngắm/nhãn bị đẩy lệch ra ngoài vùng nhìn thấy của
     `.stage-inner` (đang có overflow:hidden) → BỊ CẮT MẤT ở bên phải/dưới.
     Lấy đúng `stage.clientWidth` hiện tại làm mốc để canvas luôn được vẽ ở
     ĐÚNG kích thước sẽ hiển thị, không cần CSS co thêm nữa, nên toạ độ nội
     bộ và toạ độ CSS luôn khớp nhau trên mọi cỡ màn hình. */
  function getMaxDisplayWidth() {
    return Math.max(200, Math.floor(stage.clientWidth) || 640);
  }

  function recomputeBaseScale() {
    if (!img) return;
    const maxW = getMaxDisplayWidth();
  ...
  // Định vị lại kính ngắm + quy đổi vùng đã khoanh sau khi tỷ lệ hiển thị
  // đổi (do zoom HOẶC do đổi cỡ màn hình) — tách ra dùng chung cho cả zoom
  // (rescale) và resize cửa sổ/xoay màn hình (bên dưới), tránh lặp code.
  function reflowAfterRescale(relX, relY, oldFactor) {
  ...
  function rescale() {
    ...
    renderCanvas();
    reflowAfterRescale(relX, relY, oldFactor);
  }

  // Xoay màn hình / đổi cỡ cửa sổ (điện thoại) làm bề rộng khả dụng của
  // .stage thay đổi — cần tính lại baseScale theo bề rộng mới, nếu không
  // canvas vẫn giữ nguyên kích thước cũ và có thể lại bị CSS co nhỏ, làm
  // kính ngắm lệch/bị che như lỗi ban đầu. Debounce 150ms để không tính
  // lại liên tục khi trình duyệt bắn nhiều sự kiện resize liên tiếp lúc xoay.
