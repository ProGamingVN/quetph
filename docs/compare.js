(() => {
  ...
  function createStage(prefix) {
    ...
    const stageEl = document.getElementById('stage' + prefix);
    ...
    /* Chiều rộng hiển thị tối đa của canvas — PHẢI lấy đúng bề rộng thật sự
       mà khung .stage nhường cho canvas, KHÔNG hard-code 1 số cố định như
       trước (460). Cùng lý do như trang Quét mẫu (script.js): kính ngắm
       được định vị bằng toạ độ PIXEL NỘI BỘ của canvas (canvas.width), coi
       1 canvas-px = 1 CSS-px hiển thị — chỉ đúng khi canvas không bị CSS
       `max-width:100%` co nhỏ thêm lần nữa. Trên điện thoại (khung .stage
       hẹp hơn 460px nhiều), điều này khiến kính ngắm bị đẩy lệch ra ngoài
       `.stage-inner` (có overflow:hidden) → bị CẮT MẤT. */
    function getMaxDisplayWidth() {
      return Math.max(200, Math.floor(stageEl.clientWidth) || 460);
    }

    // Tính lại kích thước hiển thị + vẽ lại canvas theo bề rộng khả dụng
    // HIỆN TẠI — dùng chung cho lần nạp ảnh đầu (setup) và khi bề rộng đổi
    // do xoay màn hình / đổi cỡ cửa sổ (reflowForResize bên dưới).
    function renderDisplay() {
      const maxW = getMaxDisplayWidth();
      const s = state.img.width > maxW ? maxW / state.img.width : 1;
      state.scale = s;
      const w = Math.round(state.img.width * s), h = Math.round(state.img.height * s);
    ...
    // Xoay màn hình / đổi cỡ cửa sổ làm bề rộng khả dụng của .stage đổi —
    // tính lại kích thước hiển thị theo bề rộng mới, đồng thời giữ nguyên
    // VỊ TRÍ TƯƠNG ĐỐI của kính ngắm (nếu đang ở chế độ Điểm) thay vì để nó
    // lệch theo toạ độ cũ.
    function reflowForResize() {
      if (!state.img) return;
      const relX = state.pos.x / canvas.width;
      const relY = state.pos.y / canvas.height;
      renderDisplay();
      octx.clearRect(0, 0, overlay.width, overlay.height);
      state.pos = { x: relX * canvas.width, y: relY * canvas.height };
      updateReticleVisual();
      if (sampleMode === 'point') samplePoint();
    }
  ...
  // từng khung .stage thay đổi — tính lại cả 3 khung A/B/C, nếu không canvas
  // vẫn giữ nguyên kích thước cũ và có thể lại bị CSS co nhỏ, làm kính ngắm
  // lệch/bị che như lỗi ban đầu. Debounce 150ms để không tính lại liên tục.
