(() => {
  ...
  function explainCameraError(e) {
    ...
    switch (name) {
      ...
      case 'TrackStartError':
        return 'Camera đang bị ứng dụng/tab khác chiếm dụng (hoặc chưa kịp nhả ra từ lần dùng trước).<br>Hệ thống đã tự thử lại vài lần — nếu vẫn lỗi: đóng hẳn các app/tab khác đang dùng camera (kể cả app chạy nền), hoặc bấm nút "Thử lại" bên dưới sau khi đóng.';
  ...
  async function startCamera(preferDeviceId) {
    ...
    try {
      stream = await getUserMediaWithRetry({ audio: false, video: primary });
    } catch (e) {
      // Hai tình huống cần thử lại với constraint đơn giản nhất (video: true,
      // không kèm facingMode) trước khi báo lỗi hẳn:
      // (1) constraint quá chặt — không có camera sau / deviceId không còn
      //     hợp lệ (đặc biệt laptop/tablet).
      // (2) NotReadableError sau khi đã retry vẫn thất bại — trên một số
      //     Chrome Android, chính constraint facingMode:{ideal:'environment'}
      //     khiến Camera2 API trả lỗi NotReadableError giả (không phải thật
      //     sự bị chiếm dụng), trong khi yêu cầu đơn giản nhất lại mở
      //     được bình thường.
      const retryable = e && (
        e.name === 'OverconstrainedError' || e.name === 'ConstraintNotSatisfiedError' ||
        ((e.name === 'NotReadableError' || e.name === 'TrackStartError') && !preferDeviceId)
      );
      if (retryable) {
        try {
          stream = await getUserMediaWithRetry({ audio: false, video: true });
        } catch (e2) {
          cameraWanted = false;
          showCameraError(explainCameraError(e2));
          return false;
        }
      } else {
        cameraWanted = false;
