(() => {
  ...
  //
  // TỰ ĐỘNG thử lại toàn bộ quy trình này tới 3 lần (có chờ giữa các
  // lần) trước khi bỏ cuộc — trên di động, lỗi mở camera thường chỉ là
  // do hệ điều hành/trình duyệt chưa kịp nhả hẳn camera phiên trước
  // (thoáng qua), nên tự thử lại nhiều lần thường tự hết mà KHÔNG cần
  // người dùng phải bấm "Thử lại" bằng tay nhiều lần như trước.
  ...
  function renderLog() {
    ...
    logList.querySelectorAll('.copyRgbBtn').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        if (!l) return;
        const phRaw = window.prompt('pH của mẫu này?');
        if (phRaw === null) return; // người dùng bấm Huỷ — không copy
        const phVal = parseFloat(String(phRaw).replace(',', '.'));
        if (!Number.isFinite(phVal)) {
          window.alert('Giá trị pH không hợp lệ — hãy nhập 1 con số (ví dụ: 1.11).');
          return;
        }
