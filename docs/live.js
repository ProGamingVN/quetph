(() => {
  ...
  let lastPreferDeviceId = undefined;
  let retryTimer = null;
  ...
  function showCameraError(html) {
    ...
    if (retryBtn) retryBtn.addEventListener('click', () => { cameraWanted = true; startCamera(lastPreferDeviceId); });
  }
  ...
  // Tự dò camera: ưu tiên tìm đúng camera SAU (không chỉ tin facingMode
  // đơn thuần, vì 1 số thiết bị báo "environment" giả — xin thành công
  // nhưng thực ra trả về camera trước), rồi mới chấp nhận bất kỳ camera
  // nào dùng được để KHÔNG báo lỗi nếu không tìm được camera sau thật.
  async function openAnyCamera() {
    let fallbackStream = null; // camera đã mở được nhưng chưa chắc là camera sau

    // Bước 1: thử yêu cầu trực tiếp camera sau qua facingMode.
    try {
      ...
      if (!isDefinitelyFrontFacing(s)) return s; // đúng camera sau (hoặc không xác định được là trước) — dùng luôn
      fallbackStream = s; // bị trả về camera trước do bug — giữ tạm, tìm camera sau thật bên dưới
    } catch (e) {
      if (e && (e.name === 'NotAllowedError' || e.name === 'PermissionDeniedError' || e.name === 'SecurityError')) throw e;
    }

    // Bước 2: đã có quyền (thành công hoặc lỗi thiết bị bận đều đã cấp
    // quyền) — liệt kê đầy đủ camera kèm tên, thử từng camera có tên gợi
    // ý là camera sau trước tiên.
    ...
    for (const d of [...backCandidates, ...unknownCandidates]) {
      try {
        ...
        if (!isDefinitelyFrontFacing(s)) {
          stopStreamTracks(fallbackStream);
          return s; // tìm được camera không phải camera trước — dùng ngay
        }
        stopStreamTracks(s); // vẫn là camera trước, thử tiếp
      } catch (e) { /* thử camera kế tiếp */ }
    }

    // Không tìm được camera sau nào khác — dùng tạm camera đã mở được ở
    // bước 1 (dù là camera trước) để KHÔNG báo lỗi — vẫn tốt hơn là không
    // quét được gì. Người dùng có thể bấm "Đổi camera" để chuyển thủ công.
    if (fallbackStream) return fallbackStream;

    // Bước 1 lỗi hẳn (không mở được bằng facingMode) — thử thêm facingMode
    // user, rồi video:true chung, rồi từng camera theo deviceId (kể cả
    // camera đã nhận diện là "trước" ở trên) như phương án cuối.
    const lastResorts = [
  ...
  async function startCamera(preferDeviceId) {
    ...
    cameraWanted = true;
    lastPreferDeviceId = preferDeviceId;
    ...
    try {
      if (preferDeviceId) {
        // Người dùng bấm "Đổi camera" — yêu cầu đúng camera họ chọn, không
        // tự dò sang camera khác.
        stream = await getUserMediaWithRetry({ audio: false, video: { deviceId: { exact: preferDeviceId } } });
      } else {
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
      ...
      startCamera(lastPreferDeviceId);
    }
  });
})();
