(() => {
  ...
  async function openAnyCamera() {
    ...
    // ra các camera có tên gợi ý là camera SAU. Nếu có camera đã ghi nhớ là
    // dùng được từ lần trước (khớp label), đưa nó lên THỬ ĐẦU danh sách
    // để thử trước tiên, rồi mới đến các camera còn lại theo thứ tự gốc.
    ...
    let candidates = [...backCandidates, ...unknownCandidates];
    ...
    for (const d of candidates) {
      ...
      try {
        ...
        s = await navigator.mediaDevices.getUserMedia({ audio: false, video: { deviceId: { exact: d.deviceId } } });
        if (!isDefinitelyFrontFacing(s) && await verifyStreamWorks(s)) {
    ...
    // Không camera sau nào trong danh sách hoạt động — nếu bootstrap ở
    // bước 1 dù sao cũng còn dùng được (có khung hình thật, dù không chắc
    // là camera sau) thì dùng tạm, còn hơn là không quét được gì.
  ...
  // Ngưỡng cỡ khung mẫu BẮT BUỘC để được copy RGB log — khung mẫu phải
  // đặt đúng 30% (mức nhỏ nhất của thanh trượt) thì việc đo màu mới
  // đồng nhất với quy trình Calibration (tỉ lệ vùng lõi/nền giữa các lần
  // đo khác nhau nếu khung mẫu khác nhau → RGB copy ra không so được với
  // nhau/với mốc calibration đã có).
  const REQUIRED_SQUARE_PERCENT_FOR_COPY = 30;
  ...
  function renderLog() {
    ...
    logList.innerHTML = `
      <div class="hint-bubble" style="margin-bottom:10px; font-size:10.5px;">
        ⚠️ Chú ý: nút "Copy RGB" chỉ bật khi ảnh đó được chụp lúc cỡ khung mẫu đang
        đặt đúng <b>${REQUIRED_SQUARE_PERCENT_FOR_COPY}%</b> (kéo thanh trượt "Cỡ khung mẫu" về mức nhỏ nhất
        trước khi chụp) — để RGB copy ra đồng nhất tỷ lệ vùng đo với các điểm
        Calibration đã lưu, tránh so nhầm giữa các lần đo có khung mẫu khác nhau.
      </div>
    ` + logs.map((l, i) => {
      const canCopy = l.squarePercent === REQUIRED_SQUARE_PERCENT_FOR_COPY;
      ...
      <div style="padding:8px 0; border-bottom:1px solid var(--line);">
        ...
        <div style="display:flex; align-items:center; gap:8px; margin-top:4px; flex-wrap:wrap;">
          ...
          <button type="button" class="ghost copyRgbBtn" data-index="${i}" style="padding:3px 8px; font-size:10px;" ${canCopy ? '' : 'disabled'}
            title="${canCopy ? 'Copy RGB lõi và nền của lần đo này' : `Chỉ copy được khi khung mẫu = ${REQUIRED_SQUARE_PERCENT_FOR_COPY}% lúc chụp (lần này chụp lúc khung = ${l.squarePercent}%)`}">
            📋 Copy RGB
          </button>
        </div>
      </div>
    ...
    logList.querySelectorAll('.copyRgbBtn').forEach(btn => {
      ...
      btn.addEventListener('click', async () => {
        ...
        if (!l) return;
        const text = `sampleRGB: [${l.rawCoreRgb.join(', ')}], whiteRGB: [${l.rawBgRgb.join(', ')}]`;
