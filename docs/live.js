(() => {
  ...
  async function startCamera(preferDeviceId) {
    ...
    try {
      ...
      // Nếu lỗi do constraint quá chặt (không có camera sau / deviceId không
      // còn hợp lệ) — thử lại với yêu cầu camera đơn giản nhất trước khi báo
      // lỗi hẳn, vì nhiều thiết bị (đặc biệt laptop/tablet) không có
      // facingMode "environment" nhưng vẫn có camera dùng được bình thường.
      const retryable = e && (e.name === 'OverconstrainedError' || e.name === 'ConstraintNotSatisfiedError');
      if (retryable) {
        try {
