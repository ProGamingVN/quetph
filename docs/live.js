(() => {
  ...
  // Quét TUẦN TỰ toàn bộ camera ở mặt sau máy: thử mở từng camera một,
  // xác minh camera đó thực sự trả về khung hình (verifyStreamWorks) —
  // camera ĐẦU TIÊN hoạt động thật sẽ được dùng luôn, không có UI đổi
  // camera thủ công (chỉ dùng đúng 1 camera trong suốt phiên quét).
  async function openAnyCamera() {
    ...
    try {
      ...
      if (!isDefinitelyFrontFacing(s) && await verifyStreamWorks(s)) return s;
    ...
    // ra các camera có tên gợi ý là camera SAU, rồi thử LẦN LƯỢT từng cái
    // — mở, xác minh có khung hình thật, camera nào hoạt động thì dùng
    // ngay camera đó và dừng quét. THỨ TỰ ĐẢO NGƯỢC: bắt đầu từ
    // camera CUỐI CÙNG trong danh sách liệt kê rồi lùi dần về đầu —
    // theo thực tế trên máy của bạn, camera hoạt động được là camera ở
    // cuối danh sách, không phải camera 2.
    ...
    for (const d of [...backCandidates, ...unknownCandidates].reverse()) {
      let s = null;
      try {
